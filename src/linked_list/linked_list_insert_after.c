/**
 * \file linked_list_insert_after.c
 *
 * Implementation of linked_list_insert_after
 *
    ...
    linked_list_t* ll, linked_list_element_t* element, void* data)
{
    ...
    if (NULL == new_element)
    {
        return VPR_ERROR_LL_AFTER_ELEMENT_ALLOCATION_FAILED;
    }
    ...
    // if this is a copy-on-insert, then allocate space for the data and copy it
    // into that buffer.  Otherwise, set the pointer to the original data.
    if (NULL != ll->options->linked_list_element_copy)
    {
        ...
        if (NULL == data_buffer)
        {
            return VPR_ERROR_LL_AFTER_BUFFER_ALLOCATION_FAILED;
        }
